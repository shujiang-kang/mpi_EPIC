...
!  8  EVI  = VEGETATION INDEX                                              
  504 READ(KR(7),103,IOSTAT=NFL)SRAD,TMX,TMN,RFV,RHD,U10,X1,EVI,REP                        
      ...
      GO TO 507                                                                      
  508 NGN=0                                                                          
!      WRITE(KW(1),'(/T10,3I4,A,A)')IYR,MO,KDA,' RAIN, TEMP, RAD, WIND'&            !skang
...
!	      WRITE(KW(1),589)IY,MO,KDA,PDSW,FCSW             !skang                                    
          IF(MO<MOFX)GO TO 25                                                            
      121 JT1=LT(IRO,KT)                                                                 
          KOMP(KT)=1                                                                     
          IF(KT>KTMX)KTMX=KT                                                             
          ...
          IF(IHC(JT1)==NHC(7))THEN
              IF(QD>PRMT(58))THEN
!                  WRITE(KW(1),588)IY,MO,KDA,QD              !skang
                  KOMP(KT)=0                                                                     
                  CYCLE                    
              ELSE                                                       
	              CALL PSTAPP                                                                         
          ...
          IF(IHC(JT1)==NHC(27))THEN
	          ...
	          IF(NOP>0)THEN 
!	              WRITE(KW(1),63)IYR,MO1,KDA,TIL(JT1),TLA,CPNM(JJK),&                !skang
!                  X1,X2
              END IF                                                         
              IF(KFL(20)>0)THEN                                                    
      ...
      469 IF(LUN/=35)CALL TLOP(CSTX,COX,JRT)                                             
          ...
          IF(IFD/=0.AND.DKHL>0.)THEN                                                     
              DHT=DKHL                                                                     
!              IF(NOP>0)WRITE(KW(1),92)IYR,MO,KDA,DHT,DKIN,XHSM                             !skang
          END IF                                                                         
          COST=COST+CSTX                                                                 
          CSFX=CSFX+COX                                                                  
          JT2=JT1
   ...
   30 IF(LUN/=35)CALL NPCY
	  ...
	  DO IN2=1,IGO                                                                    
          ...
          IF(JPL(JJK)>0)THEN
              ...
              JPL(JJK)=0                                                                     
!              IF(NOP>0)WRITE(KW(1),89)IYR,MO,KDA,PDSW,HU(JJK),XHSM,CPNM(JJK)                           !skang
              HU(JJK)=0.                                                                     
...
!     WRITE MO VALUES AND SUM YEARLY VALUES                                          
      ...
      IF(LMS==0.AND.TLA<1.E-5)THEN
          ...
          IF(TLA>0.)THEN                                                                 
	          ...
	          IF(NOP>0)THEN 
!	              WRITE(KW(1),63)IYR,MO1,KDA,TIL(IAUL),TLA,CPNM(JJK),&                        !skang
!                  X1,X2
              END IF                                                         
              IF(KFL(20)>0)THEN                                                    
                  WRITE(KW(20),567)IYR,MO1,KDA,KDC(JJK),X3,X3,TLA                            
      ...
      IF(IY==IPY)THEN
          II=IPYI                                                                        
          IF(IPD<=2)THEN
!              WRITE(KW(1),96)IYR,(HED(KYA(K)),SMY(KYA(K)),K=1,NKYA)                          !skang
          ELSE
...
!                  WRITE(KW(1),143)HEDP(L),(SMMP(L,K,J),J=1,12),&                   !skang
!                        SMYP(L,K),HEDP(L)             
              END DO                                                                         
              DO L=8,9                                                                       
