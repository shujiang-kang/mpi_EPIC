...
!          write(*,*) 'the job is done'
    ...
    SUBROUTINE mastercode()
!     USE parallel
      USE mpi
      USE parm


      INTEGER :: nread       ! Number of rows read from EPICRUN
      INTEGER :: adata(nruns_per_job,nparams_per_run)
      INTEGER :: cntr
      INTEGER :: worker, work_tag,shut_down
      INTEGER :: active_workers ! Number of workers currently assigned jobs
      INTEGER :: status(MPI_STATUS_SIZE)
      INTEGER :: adata2(nparams_per_run, nruns_per_job)
      INTEGER :: send_buf(nruns_per_job*nparams_per_run)
      
      ! Create output folders for each processor use comm_size !skang
      ! Open EPICRUN.DAT

      CALL OPENV(KR(11),"EPICRUN.DAT",KW(MSO))

      ! While not the end of EPICRUN
      ...
      DO worker = 1, tasks-1
         ! Load nruns_per_job runs from EPICRUN
       
         CALL READEPICRUNS(nread, adata)

         IF (nread > 0) THEN
             ! send job data and number of jobs
             !call runs_send(worker, adata, nread)
             adata2=transpose(adata)
             send_buf = reshape(adata2(:,:),shape = (/nruns_per_job*nparams_per_run/)) 
  
             call mpi_send(send_buf, size(send_buf), mpi_integer, worker, work_tag, mpi_comm_world, ierr)   
             
         END IF
      
        ! if ((worker > tasks) .OR. (nread < nruns_per_job)) exit
        ! if (nread < nruns_per_job) exit
       END DO
      

      ! While not the end of EPICRUN
      DO
!        call runs_nextnode(worker)
         call mpi_recv(a,1,mpi_integer, MPI_ANY_SOURCE, MPI_ANY_TAG, mpi_comm_world, status, ierr)
!        call mpi_probe(MPI_ANY_SOURCE, work_tag, mpi_comm_world, status, ierror)         
...
!         call shutdown_node(worker)  ! No more work, so send shut down

    END SUBROUTINE mastercode

    SUBROUTINE slavecode()
!      USE parallel
       USE mpi
       USE parm

!       integer :: worker, ierror
        integer :: worker
      ! Loop until receive exit flag
      INTEGER :: nread       ! Number of rows read from EPICRUN  
      INTEGER :: adata(nruns_per_job*nparams_per_run)
      INTEGER :: cntr
      INTEGER :: run_buf(nruns_per_job, nparams_per_run) 
      integer :: a =0
      integer :: recv_tag, status(MPI_STATUS_SIZE)
      
     
     ! While not the end of EPICRUN
      DO
!        call runs_recv(adata, nread)
         call mpi_recv(adata, size(adata), mpi_integer, 0,MPI_ANY_TAG, mpi_comm_world,status, ierr)
         ...
         nread = nruns_per_job 
         run_buf = reshape(adata, shape(run_buf), order=(/2,1/))
         IF (nread > 0) THEN
             do cntr=1,nread
                ! call runepic(adata(cntr,:))
                ! if (run_buf(cntr,1) == 0) exit   
                call runepic(run_buf(cntr,:)) 
             end do

!             call runs_nodefinished()
           call mpi_send(a,1,mpi_integer, main_rank, 0, mpi_comm_world, ierr)
	...
	END SUBROUTINE slavecode

	SUBROUTINE READEPICRUNS(nread, adata)
        USE parm
!       USE parallel 

	INTEGER, INTENT(INOUT) :: nread
	INTEGER, INTENT(INOUT) :: adata(nruns_per_job, nparams_per_run)
        INTEGER :: cntr, fstatus

        cntr = 0
        fstatus = 0
!       nread = 30
    
        DO WHILE ((cntr < nread) .and. (fstatus == 0))
            !  1  ASTN = RUN #
            !  2  ISIT = SITE #
            ...
            READ(KR(11),'(I8,8I6)',IOSTAT=fstatus) adata(cntr+1,:)
            if (fstatus == 0) cntr = cntr + 1
        END DO

        nread = cntr
	END SUBROUTINE READEPICRUNS

	SUBROUTINE runepic(rundata)
      
      USE parm 
!      USE parallel      
      INTEGER::rundata(8)   ! Holds one row of data from EPICRUN
      CHARACTER(8)::run_number		                                                 
      CHARACTER(1)::ASG 
...
! DManowitz - 2/26/10: For help in comparing output between Windows & Linux, changing some outputs.
      ...
      CHARACTER(32)::RUNID,RUNSF,RUNS,RUNSFDIR  ! added by skang
      ...
      DO I=1,SIZE(KW)                                                                          
   	     ! KW(I)=I+50
             KW(I)=I+50+this_rank*1000 ! changed by SKang
...
!     LINE 1/2                                                                       
      ...
      ELSE
         ...
         ! WRITE(RUNID,'(I6)')this_rank
          !RUNS = "EPICRUN"
         ! RUNID = ADJUSTL(RUNID)       
         ! RUNSF = 'EPICRUN'//TRIM(RUNID)
         ! RUNSFDIR =TRIM(RUNSF)//'/'
         ! RUNS =TRIM(RUNSF)//'.DAT'
         ! RUNS = TRIM(RUNS)//'.DAT'

         ! CALL  MAKEDIRQQ(RUNSF) 
              
         ! CALL OPENV(KR(11),RUNS,KW(MSO)) !?????                                             
...
!  519 DO 740
 ...
 !         IF(NFL/=0)GO TO 219                                                            
     ...
     ! WRITE(*,679)TRIM(ASTN),ISIT,IWP1,INPS,IOPS,IWTH     !skang
     ! WRITE(KW(1),679)TRIM(ASTN),ISIT,IWP1,INPS,IOPS,IWTH !skang                                          
      K19=0 
     ...
     !  OUTDIR=trim(OUTDIR)
      ! SKang 3/19/2012
      CALL OPENF
     ! WRITE(KW(1),621) VersionStr,TRIM (CommandName),IYER,IMON,IDAY,IT1,IT2,IT3     !skang 
     ...
     !    TRIM(FPRNT), 'FCMOD', TRIM(FCMOD), 'FWLST', TRIM(FWLST)  !skang
      WPM1FILE=' '                                                                   
      WINDFILE=' '                                                                   
      UAV0=0.                                                                        
      NYD=1                                                                          
      ...
      IRO0=1                                                                         
      IRUN=IRUN+1                                                                    
     ! WRITE(KW(1),'(/1X,A)')'-----VARIABLE NAMES'                                    !skang
     ! WRITE(KW(1),229)                                                               !skang
     ! WRITE(KW(1),287)HED                                                            !skang
     ! WRITE(KW(1),245)                                                               !skang
     ! DO J=1,30                                                                      !skang
         ! WRITE(KW(1),242)J,PRMT(J),(SCRX(J,I),I=1,2),(SCRP(J,I),I=1,2)              !skang  
     ! END DO                                                                         !skang
     ! DO J=31,SIZE(PRMT)                                                             !skang        
     !     WRITE(KW(1),242)J,PRMT(J)                                                  !skang  
     ! END DO                                                                         !skang
      II=-1                                                                      
      DO WHILE(II/=ISIT)                                                                            
...
!      WRITE(KW(1),'(//1X,A/)')'____________________WEATHER DATA________________________'
      ...
      ELSE                  
	      IF(ICO2==1)THEN                                                                     
              ...
              END IF                                                                       
              !WRITE(KW(1),'(T10,A)')'DYNAMIC ATMOSPHERIC CO2'                              !skang
          ELSE                                                                           
	      !    WRITE(KW(1),'(T10,A)')'INPUT ATMOSPHERIC CO2'                            !skang         
	      END IF                                                                              
      END IF
     ! WRITE(KW(1),'(T10,A,F6.0,A)')'CO2 CONC ATMOSPHERE = ',CO2,' ppm'               !skang
     ! WRITE(KW(1),'(T10,A,F4.0,A)')'PERIOD OF RECORD P5MX =',YWI,' Y'                !skang
      IF(NGN>0)THEN                                                                  
          CALL WIGV                                                                    
      ELSE                                                                           
     !     WRITE(KW(1),'(/T10,A,A)')'**********RAIN, TEMP, RAD, WIND SPEED,',&         !skang    
...
!          WRITE(KW(1),295)                                                               !skang
      ...
      IF(IW>1)THEN                                                                   
	      II=IW-1                                                                           
!          WRITE(KW(1),571)II,TITW5(1)                                                  !skang
	  ELSE                                                                                
!          WRITE(KW(1),602) TRIM(WPM1FILE)                                              !skang
      END IF                                                                         
!      WRITE(KW(1),321)HED(1),(OBMX(IW,I),I=1,12),XYP(1),HED(1)                         !skang
!     WRITE(KW(1),321)HED(2),(OBMN(IW,I),I=1,12),XYP(2),HED(2)                          !skang
!      WRITE(KW(1),224)'SDMX',(SDTMX(IW,I),I=1,12),'SDMX'                               !skang
!      WRITE(KW(1),224)'SDMN',(SDTMN(IW,I),I=1,12),'SDMN'                               !skang
!      WRITE(KW(1),243)HED(4),(RMO(IW,I),I=1,12),XYP(3),HED(4)                          !skang
!      WRITE(KW(1),223)'SDRF',(RST(2,IW,I),I=1,12),'SDRF'                               !skang
...
!      WRITE(KW(1),224)' PCF',(PCF(IW,I),I=1,12),' PCF'                                !skang
      IF(IWP5==0)EXIT                                                                
  568 CONTINUE                                                                       
      REWIND KR(18)
      IF(IWND==0)THEN
...
!          WRITE(KW(1),'(T10,A,F8.3)')'EQUIVALENT LATITUDE = ',YLAZ  !skang
      ELSE
          YLAZ=YLAT          
      END IF
      XX=YLAZ/CLT
...
!     LINE 3                                                                         
	  ...
	  ELSE                                                                                
          CLF=1.                                                                       
	  END IF                                                                              
!      WRITE(KW(1),'(T10,A,F7.3)')'CLIMATIC FACTOR = ',CLF                            !skang
...
!     LINES 4/19                                                                     
	  ...
	  END DO                                                                              
      REWIND KR(25)                                                                  
!      WRITE(KW(1),'(/T20,2A)')'WIND = ', TRIM(WINDFILE)                              !skang
!      WRITE(KW(1),321)HED(7),(UAVM(I),I=1,12),AWV,HED(7)                             !skang
!      IF(ICDP==0)THEN                                                                
...
!      END IF                                                                         !skang
      AHSM=CAHU(1,365,0.,1)                                                          
!      WRITE(KW(1),221)SUM,AHSM                                                       !skang
!      WRITE(KW(1),'(//1X,A)')'-----WIND EROSION DATA'                                !skang
!      WRITE(KW(1),285)FL,FW,ANG0,UXP,DIAM,ACW                                        !skang
      DO I=1,12                                                                   
          IF(UAVM(1)>0.)THEN                                                             
              CALL AEXINT(UXP,SUM)                                                         
              UAVM(I)=UAVM(I)/SUM                                                          
      ...
      DST0=TX                                                                        
      CALL APAGE(0)                                                                  
!      WRITE(KW(1),'(//1X,A/)')'____________________GENERAL INFORMATION______________________'  !skang
      KRX=KR(16)                                                                     
      IF(NSTP>0)THEN                                                                 
!	      WRITE(KW(1),'(T10,A)')'REAL TIME SIMULATION MODE'                                 !skang
	      ...
	      END IF                                                                            
	  ELSE                                                                                
!          WRITE(KW(1),'(T10,A)')'NORMAL SIMULATION MODE'                                        !skang
      END IF                                                                         
!      IF(PRMT(50)>0.)THEN                                                            
!	      WRITE(KW(1),'(T10,A)')'DYNAMIC TECHNOLOGY'                                         !skang
!	  ELSE                                                                                
!	      WRITE(KW(1),'(T10,A)')'STATIC TECHNOLOGY'                                          !skang
!  	  END IF                                                                            
!      WRITE(KW(1),301)NBYR,IYR0,IMO0,IDA0                                                       !skang
!      IF(LPYR>0)THEN                                                                 
!	      WRITE(KW(1),'(T10,A)')'LEAP YEAR IGNORED'                                         !skang
!	  ELSE                                                                                
!          WRITE(KW(1),'(T10,A)')'LEAP YEAR CONSIDERED'                                 
!      END IF                                                                         
!      WRITE(KW(1),365)WSA,YLAT,XLOG,ELEV,CHL,CHS,CHN,CHD,UPSL,UPS,AZM,SN                        !skang
!      WRITE(KW(1),'(T10,A,A4)')'WATER EROSION FACTORS--DRIVING EQ = ',&                         !skang
!      HED(NDVSS)                                                                     
      IF(NDRV==6)WRITE(KW(1),'(T15,A,4E13.5)')'MUSI COEFS = ',BUS                    
      BUS(1)=BUS0*WSX**BUS(4)                                                        
!      WRITE(KW(1),405)SL,RXM,RLF,RSF,TC                                                         !skang
!      WRITE(KW(1),'(T10,A)')'DAILY RUNOFF ESTIMATION'                                           !skang
!      SELECT CASE(INFL)                                                              
!	      CASE(1)                                                                           
!              WRITE(KW(1),'(T15,A)')'NRCS CURVE NUMBER EQ'                                      !skang
!          CASE(2)                                                                      
!              WRITE(KW(1),'(T15,A/T15,A)')'GREEN & AMPT EQ','RF EXP DST--'&                     !skang
!              'PEAK RF RATE SIM'                                                      
!          CASE(3)                                                                      
...
!         = 9 WITHIN DAY WIND SPEED DIST                                             
      ...
      DO J=1,21                                                                      
          IX0(J)=IX(J)                                                                 
      END DO                                                                         
!      WRITE(KW(1),297)IGN,(IX(IDG(I)),I=1,10),(IDG(I),I=1,10)                        !skang
	  V3=AUNIF(IDG(3))                                                                    
      V1=AUNIF(IDG(2))                                                               
      BIG=MAX(.2,PRMT(24))                                                           
...
!     OCPD=.1*OCPD/SUM                                                               
      ...
      END IF                                                             
      CALL SPRNT(YTP)                                                                
!      WRITE(KW(1),'(//1X,A/)')'____________________SOIL DATA____________________'   !skang
...
!	  END IF                                                                                   !skang
      SATK=SATC(LID(2))
!      WRITE(KW(1),'(T10,A,F10.3)')'INITIAL STANDING DEAD = ',STD0                                 !skang                             
      II=-1
      DO WHILE(II/=IOPS)                                                                             
...
!     LINE 2                                                                         
      ...
      CALL HSGCN                                                                     
      CALL HCNSLP(CN2,X3)                                                            
!      WRITE(KW(1),390)ASG(ISG),LUN,CN2,X3,SCRP(30,1),SCRP(30,2),SCRP(4,1),&              !skang
!      SCRP(4,2)                                                                      
      CN2=X3                                                                         
      CN0=CN2                                                                        
...
!    DManowitz - 6/7/10: Always print out STL & STD for now.      
	  ...
	  END IF                                                                              
!      WRITE(KW(1),'(//1X,A/)')'____________________MANAGEMENT DATA_____________________'  !skang
!      WRITE(KW(1),'(T10,2A)')'OPSC = ',TRIM(OPSCFILE)
      NCOW=WSA/RST0                                                                                                                                                                                  
      IF(IRR==0)THEN
          VIMX=0.                                                                        
!          WRITE(KW(1),'(T10,A)')'DRYLAND AGRICULTURE'                                    !skang
          GO TO 142
      ELSE
...
!      ELSE                                                                           
!          WRITE(KW(1),'(T10,A)')'NO LIME APPLICATIONS'                                 !skang
!      END IF                                                                         
      HU(1)=0.                                                                       
      IBGN=1                                                                         
      JJK=1                                                                          
...
!          WRITE(KW(1),317)I,JX(2),JX(3),TIL(IJ),I3,JX(4),JX(5),X1,X2,EMX(IJ)&            !skang
...
!              (I,L),PSTE(I,L),HUSC(I,L)                                                      
      ...
      CALL TILTBL                                                                    
      IAUI=NDT                                                                       
!	  WRITE(KW(1),715)TIL(NDT),TLD(NDT)                                                   !skang
!     IF(IAUF==0)GO TO 689                                                           
      JX(4)=261                                                                      
      JX(5)=12                                                                       
      CALL TILTBL                                                                    
      ...
      IF(LMS==0)THEN
          ...
          CALL TILTBL                                                                    
          IAUL=NDT                                                                       
!	      WRITE(KW(1),714)TIL(NDT),TLD(NDT)                                               !skang
      END IF
      L=1                                                                            
      IF(BFT0>0.)THEN	                                                        
          IDFT(1)=IDF0                                                                   
	      ...
	      DO K=1,2                                                                       
              ...
              IDFT(K)=L                                                                    
          END DO                                                                         
!          WRITE(KW(1),716)TIL(IAUF),TLD(IAUF),FTNM(IDFT(1))                                  !skang
      END IF
      IF(IPAT>0)THEN
          IF(IDFP==0)IDFP=53
	      JX(7)=IDFP
...
!      WRITE(KW(1),281)(IDC(I),I=1,LC)                                                !skang
      IPL=0                                                                          
      LRG=0                                                                          
      DO I=1,LC                                                                  
          NHU(I)=IHU(I)                                                                  
      ...
      SMMC=0.                                                                        
!	  DO J=1,LRG                                                                          
!          WRITE(KW(1),78)'POP ',(POP(I,J),I=1,LC)                                      !skang
!          WRITE(KW(1),53)'MXLA',(PPLA(I,J),I=1,LC)                                     
!      END DO                                                                         
!      DO J=1,LRG                                                                     
!          WRITE(KW(1),35)'PHU ',(PHU(I,J),I=1,LC)                                      !skang
!	  END DO                                                                              
      CALL APAGE(1)                                                                  
!      WRITE(KW(1),348)                                                               
!      WRITE(KW(1),248)(CPNM(I),I=1,LC)                                               
...
!      WRITE(KW(1),'(/T10,A,F7.2,A)')'FINAL WATER CONTENT OF SNOW = ',SNO,' mm'     !skang
...
!          END DO
      END DO                                                                                   
!      WRITE(KW(1),'(/1X,A)')'-----AVE ANNUAL VALUES (g/ha)'                              !skang
      DO K=1,NDP                                                                     
          DO L=1,7                                                                     
...
!      WRITE(KW(1),383)(PSTN(K),K=K2,N1)                                                 !skang
      IF(MASP==0)GO TO 467                                                           
!      WRITE(KW(1),384)HEDP(1),(SMAP(1,K),K=K2,N1)                                       !skang
      I=1                                                                            
      DO K=K2,N1                                                                     
          PPX(I)=SMAP(2,K)                                                             
          ...
          I=I+1                                                                        
      END DO                                                                         
!      WRITE(KW(1),387)HEDP(2),(PPX(I),I=1,N2)                                           !skang
      I=1                                                                            
      DO K=K2,N1                                                                     
          PPX(I)=SMAP(3,K)                                                             
          ...
          I=I+1                                                                        
      END DO                                                                         
!      WRITE(KW(1),387)HEDP(3),(PPX(I),I=1,N2)                                           !skang
      I=1                                                                            
      DO K=K2,N1                                                                     
          PPX(I)=SMAP(4,K)                                                             
          ...
          I=I+1                                                                        
      END DO                                                                         
!      WRITE(KW(1),387)HEDP(4),(PPX(I),I=1,N2)                                           !skang
      DO L=5,7                                                                       
!          WRITE(KW(1),384)HEDP(L),(SMAP(L,K),K=K2,N1)                                   !skang
      END DO                                                                         
      I=1                                                                            
      DO K=K2,N1                                                                     
          PPX(I)=SMAP(10,K)                                                            
          ...
          I=I+1                                                                        
      END DO                                                                         
!      WRITE(KW(1),387)HEDP(10),(PPX(I),I=1,N2)                                          !skang
      GO TO 469                                                                      
  467 DO L=1,7                                                                       
